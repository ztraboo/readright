#!/usr/bin/env bash
# run_generate_word_mp3s.sh
# Lightweight helper to create/use a Python venv, ensure `elevenlabs` is installed,
# and run scripts/generate_word_mp3s.py.
# Usage: ./scripts/run_generate_word_mp3s.sh [--venv /path/to/venv] [--pip-extra-args "..."] [-- python-args]
...
then runs scripts/generate_word_mp3s.py with any extra args forwarded.

Examples:
  ./scripts/run_generate_word_mp3s.sh
  ./scripts/run_generate_word_mp3s.sh -- pip_arg1 pip_arg2
  VENV_DIR=/tmp/myenv ./scripts/run_generate_word_mp3s.sh
...
PY_SCRIPT="$REPO_ROOT/scripts/generate_word_mp3s.py"
if [[ ! -f "$PY_SCRIPT" ]]; then
